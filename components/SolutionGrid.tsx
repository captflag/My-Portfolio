...
const PROTOCOLS = [
  ...
  {
    id: 'lead-gen',
    title: 'Human-Emulated Discovery',
    code: 'AGENT_SCRAPE_X',
    description: 'Advanced entity extraction that bypasses modern anti-bot protocols.',
    advantage: 'Anti-Detection Alpha',
    icon: Search,
    colSpan: 'md:col-span-1',
    caseStudy: 'Multi-agent system automated 90% of a B2B sales cycle for a YC-backed startup.',
    metric: '₹12L pipeline/wk',
    logicSteps: ['Target Recon', 'Entity Extract', 'Social Proof', 'Lead Qual'],
    details: "Generic scrapers get blocked. Our agents utilize dynamic fingerprinting and behavioral modeling to appear as human researchers."
  },
  {
    id: 'chatbots',
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-4 no-scrollbar">
        {steps.map((step, i) => (
          <React.Fragment key={step}>
            <div className="shrink-0 p-3 bg-cyber-grey/20 border border-white/10 rounded-lg text-[10px] font-mono text-white/80 uppercase whitespace-nowrap">
              {step}
            </div>
            {i < steps.length - 1 && (
              <motion.div 
  ...
  return (
    ...
    >
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex flex-col">
            <div className="flex gap-2 items-center mb-1">
              <div className="text-[9px] font-mono text-cyber-blue flex items-center gap-2">
                <Activity size={10} className="animate-pulse" />
                ACTIVE
              </div>
              <div className="px-2 py-0.5 rounded bg-cyber-blue/20 border border-cyber-blue/40 text-[7px] font-bold text-cyber-blue uppercase tracking-widest">
                v5.0_CORE
              </div>
            </div>
            <h3 className="text-xl md:text-2xl font-black uppercase text-white tracking-tighter leading-none">
              {protocol.title}
            </h3>
          </div>
          <Icon className={`w-6 h-6 md:w-8 md:h-8 transition-all duration-500 ${isHovered ? 'text-cyber-blue scale-110 drop-shadow-[0_0_10px_rgba(201,41,36,0.6)]' : 'text-slate-600'}`} />
        </div>
      ...
      <div className="mt-auto relative z-10 flex flex-col gap-4">
        <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-[0.2em] text-cyber-blue">
    ...
    <section className="py-16 md:py-24 px-4 md:px-20 bg-cyber-black relative" id="protocols">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="mb-12 md:mb-16 flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-8 gap-6">
          <div>
            <div className="text-cyber-blue font-mono text-[10px] md:text-xs uppercase tracking-[0.4em] mb-4">Strategic_Differentiators</div>
            <h2 className="text-3xl md:text-7xl font-sans font-black uppercase tracking-tighter leading-[0.9] text-white">
              Why CAPT AI<br />
          ...
          <div className="text-left md:text-right">
            ...
            <div className="text-2xl md:text-4xl font-black text-white">+314%</div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {expandedProtocol && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[150] bg-cyber-black/98 backdrop-blur-2xl flex items-center justify-center p-2 md:p-12"
            onClick={() => setExpandedProtocol(null)}
          >
            <motion.div
              layoutId={`card-${expandedProtocol.id}`}
              className="glass w-full max-w-5xl max-h-[95vh] rounded-2xl md:rounded-3xl overflow-y-auto border border-cyber-blue/30 flex flex-col md:flex-row relative shadow-[0_0_100px_rgba(201,41,36,0.2)]"
            ...
            >
              ...
              <div className="w-full md:w-3/5 p-6 md:p-12 flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 relative z-10">
                <div className="text-cyber-blue font-mono text-[10px] md:text-xs uppercase mb-4 flex items-center gap-2">
                  <Code size={12} /> {expandedProtocol.code} // CASE_LOG_77
                </div>
